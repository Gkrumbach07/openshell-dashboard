...
func (app *App) ListServices(w http.ResponseWriter, r *http.Request) {
	services, err := app.client.Services().List(r.Context(), chi.URLParam(r, "workspace"), chi.URLParam(r, "name"))
	if err != nil {
		writeSDKError(w, err)
		return
	}
	...
	for _, svc := range services {
		out = append(out, models.FromServiceEndpoint(svc))
	}
...
func (app *App) ExposeService(w http.ResponseWriter, r *http.Request) {
	...
	svc, err := app.client.Services().Expose(r.Context(), chi.URLParam(r, "workspace"), chi.URLParam(r, "name"), body.Service, body.TargetPort, body.Domain)
	if err != nil {
		writeSDKError(w, err)
		return
	}
	writeJSON(w, http.StatusCreated, models.FromServiceEndpoint(svc))
}

func (app *App) DeleteService(w http.ResponseWriter, r *http.Request) {
	if err := app.client.Services().Delete(r.Context(), chi.URLParam(r, "workspace"), chi.URLParam(r, "name"), chi.URLParam(r, "svc")); err != nil {
		writeSDKError(w, err)
		return
	}
