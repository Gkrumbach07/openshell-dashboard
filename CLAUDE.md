...
- **Relay-only auth** ([ADR 0014](docs/adrs/0014-relay-only-bff.md), [ADR 0001](docs/adrs/0001-three-mode-auth-architecture.md)). The BFF never terminates authentication — a fronting proxy (oauth2-proxy standalone, kube-auth-proxy federated) owns login/sessions/refresh/CSRF and injects `x-forwarded-access-token`. Bearer chain: proxy header → `Authorization: Bearer` → 401. The BFF never validates tokens and never authorizes ([ADR 0003](docs/adrs/0003-proxy-delegated-auth-bff-pattern.md)). The only auth switch is `AUTH_DISABLED` (dev).
- **BFF scope boundary** ([ADR 0011](docs/adrs/0011-bff-scope-boundary.md)). The BFF has three jobs: API translation, browser-app hosting, operational surface. The never-list (no auth termination, no JWT validation, no RBAC, no k8s API, no credential brokering, no server-side state) is authoritative — cite it to reject scope creep.
