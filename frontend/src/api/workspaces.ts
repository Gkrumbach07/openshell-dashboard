...
export const listWorkspaces = (): Promise<Workspace[]> => get<Workspace[]>('/api/v1/workspaces');

export const getWorkspace = (name: string): Promise<Workspace> =>
  get<Workspace>(`/api/v1/workspaces/${encodeURIComponent(name)}`);

export const createWorkspace = (body: CreateWorkspaceRequest): Promise<Workspace> =>
  post<Workspace>('/api/v1/workspaces', body);
...
export const listMembers = (workspace: string): Promise<WorkspaceMember[]> =>
  get<WorkspaceMember[]>(`/api/v1/workspaces/${encodeURIComponent(workspace)}/members`);

export const addMember = (workspace: string, body: AddMemberRequest): Promise<WorkspaceMember> =>
  post<WorkspaceMember>(`/api/v1/workspaces/${encodeURIComponent(workspace)}/members`, body);

export const removeMember = (workspace: string, subject: string): Promise<{ removed: boolean }> =>
  del<{ removed: boolean }>(
...
export const useWorkspace = (name: string) =>
  useQuery({ queryKey: ['workspaces', name], queryFn: () => getWorkspace(name) });
...
export const useMembers = (workspace: string) =>
  useQuery({ queryKey: ['members', workspace], queryFn: () => listMembers(workspace) });

export const useAddMember = (workspace: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (body: AddMemberRequest) => addMember(workspace, body),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['members', workspace] }),
  });
};
